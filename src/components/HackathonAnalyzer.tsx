...
export const HackathonAnalyzer = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 md:space-y-8">
      <Card className="bg-gradient-card border-0 shadow-medium p-4 md:p-6">
        <Tabs defaultValue="upload" className="w-full">
          <TabsList className="grid w-full grid-cols-3 mb-4 md:mb-6 h-auto">
            <TabsTrigger value="upload" className="flex items-center gap-1 md:gap-2 text-xs md:text-sm p-2 md:p-3">
              <Upload className="w-3 h-3 md:w-4 md:h-4" />
              <span className="hidden sm:inline">File Upload</span>
              <span className="sm:hidden">Upload</span>
            </TabsTrigger>
            <TabsTrigger value="link" className="flex items-center gap-1 md:gap-2 text-xs md:text-sm p-2 md:p-3">
              <Link className="w-3 h-3 md:w-4 md:h-4" />
              <span className="hidden sm:inline">Links</span>
              <span className="sm:hidden">Links</span>
            </TabsTrigger>
            <TabsTrigger value="manual" className="flex items-center gap-1 md:gap-2 text-xs md:text-sm p-2 md:p-3">
